...
class CPUAttentionBackendImpl(AttentionImpl):
    ...
    def forward(
        ...
        if attn_metadata.use_sdpa_prefill:
            ...
            # decoding - flash attention - scheduler_metadata
            # prefil - sdpa - (K*Q) *V - no scheduler_metadata

            # decoding - flash attention - scheduler_metadata
            # prefill - flash attention  -
            # scheduler_metadata 1:1 token -> work_plan
        ...
        if num_actual_tokens > 0:
            ops.cpu_attention_with_kv_cache(
                ...
                query_start_loc=attn_metadata.query_start_loc, # update
                seq_lens=attn_metadata.seq_lens, # update
                scale=self.scale,
